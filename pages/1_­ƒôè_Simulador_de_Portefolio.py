...
def run_portfolio_analysis(tickers, weights, start_date, initial_inv, monthly_dca):
    ...
    <head>
        ...
        <style>
            ...
            h1 {{ color: #1a252f; text-align: center; margin-bottom: 20px; }}
            .container {{ max-width: 1200px; margin: 0 auto; background: #ffffff; padding: 25px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }}
            .styled-table {{ border-collapse: collapse; margin: 25px 0; font-size: 0.95em; min-width: 100%; border-radius: 8px; overflow: hidden; box-shadow: 0 0 20px rgba(0,0,0,0.05); }}
            .styled-table thead tr {{ background-color: #1f77b4; color: #ffffff; text-align: left; font-weight: bold; }}
            .styled-table th, .styled-table td {{ padding: 12px 15px; border-bottom: 1px solid #dddddd; }}
            .styled-table tbody tr:nth-of-type(even) {{ background-color: #f3f3f3; }}
            .styled-table tbody tr:last-of-type {{ border-bottom: 2px solid #1f77b4; }}
        </style>
    </head>
    <body>
        <div class="container">
            <h1>📊 Relatório Interativo de Portefólio e Dividendos vs S&P 500</h1>
...
# ------------------------------------------------------------------------------
st.title("📊 Simulador de Portefólio vs S&P 500")
st.caption("Simula um portefólio com investimento inicial + DCA mensal, comparado com o S&P 500 (com dividendos reinvestidos).")
